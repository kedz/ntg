...
from data_structs import Vocab, LengthVocab, Seq2SeqDataset, Seq2SeqSPDataset
import torch
...
def load_data_sp(train_path, valid_path, src_field=0, tgt_field=1, 
                 enc_vocab_size=100000, dec_vocab_size=100000):
    ...
    print("src vocab: {} tokens.".format(vocab_src.size))
    print("tgt vocab: {} tokens.".format(vocab_tgt.size))
    train_data = extract_sp_dataset_from_tsv(
        train_path, vocab_src, vocab_tgt)
    valid_data = extract_sp_dataset_from_tsv(
        valid_path, vocab_src, vocab_tgt)
    return (vocab_src, vocab_tgt), (train_data, valid_data)
...
def extract_sp_dataset_from_tsv(path, vocab_src, vocab_tgt, skip_header=True):
    ...
    print("allocating counts...")
    bow = torch.zeros(targets_out.size(0), vocab_tgt.size).float()
    for i in range(targets_out.size(0)):
        for j in range(target_lengths[i]):
            bow[i,targets_out[i,j]] += 1

    dataset = Seq2SeqSPDataset(
        sources, source_lengths, targets_in, targets_out, target_lengths,
        bow)
